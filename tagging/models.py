...
class TagManager(models.Manager):
    ...
    def add_tag(self, obj, tag_name, labels=None):
        """
    ...
    def usage_for_model(self, model, counts=False, min_count=None, filters=None, label=None):
        """
        ...
        Nonrel-specific note: the label argument is a string that can be used
        to perform denormalized aggregations on the data.  It is arbitrary, but
        can be assigned to tags when they are first created, and then subsequently
        queried for and counted.
        """
        if filters is None: 
    ...
    def cloud_for_model(self, model, steps=4, distribution=LOGARITHMIC,
                        filters=None, min_count=None, label=None):
        """
        Obtain a list of tags associated with instances of the given
...
class ModelTags(models.Model):
    ...
    tag_name     = models.CharField(_('tag name'), max_length=50, unique=True, db_index=True)
